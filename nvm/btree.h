...
struct BTreeLeaf : public BTreeLeafBase
{
    ...
    BTreeLeaf *split(Key &sep)
    {
        ...
        for(int i = 0; i < newLeaf->count; i++){
            newLeaf->data[i].first = data[i+count].first;
            newLeaf->data[i].second = data[i+count].second;      
        }          
...
struct BTree
{
    ...
    struct unsafe_iterator{
        ...
        bool valid() const{
            return node != nullptr && pos < node->count;
        }
        Key key() const { return node->data[pos].first; }
        Value value() const { return node->data[pos].second; }
    ...
    unsafe_iterator last_unsafe(){
        NodeBase *node = root;
        while (node->type == PageType::BTreeInner)
        {
            auto inner = static_cast<BTreeInner<Key> *>(node);
            node = inner->children[inner->count];
        }
        assert(node->type == PageType::BTreeLeaf);
       // std::cout<< node->count ;
        return unsafe_iterator((BTreeLeaf<Key, Value>*)node, node->count - 1);
    }


    unsafe_iterator seek(const Key& key){
         NodeBase *node = root;
         while (node->type == PageType::BTreeInner){
            auto inner = static_cast<BTreeInner<Key> *>(node);
            node = inner->children[inner->lowerBound(key)];
        }
        BTreeLeaf<Key, Value> *leaf = static_cast<BTreeLeaf<Key, Value> *>(node);
        int pos = leaf->lowerBound(key);
        return unsafe_iterator((BTreeLeaf<Key, Value>*)node, pos);
    }
