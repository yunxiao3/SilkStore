//
// Created by zxjcarrot on 2019-09-29.
//
...
TEST(DBTest, DestroyEmptyDir) {
    ...
    ASSERT_OK(silkstore::DestroyDB(dbname, opts));
    ASSERT_TRUE(!env.FileExists(dbname));
...
TEST(DBTest, DestroyOpenDB) {
    ...
    ASSERT_TRUE(env_->FileExists(dbname));
    ASSERT_TRUE(!silkstore::DestroyDB(dbname, Options()).ok());
    ASSERT_TRUE(env_->FileExists(dbname));
    ...
    ASSERT_OK(silkstore::DestroyDB(dbname, Options()));
    ASSERT_TRUE(!env_->FileExists(dbname));
}
